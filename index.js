...
async function run() {
  try {
    ...
    const checkBan = async (req, res, next) => {
      const email = req.body.email || req.params.email || req.decoded?.email;
      if (!email) {
    ...
    app.get("/users/admin/:email", verifyToken, checkBan, async (req, res) => {
    ...
    app.get("/admin-analytics", verifyToken, verifyAdmin, async (req, res) => {
      ...
      const donationCount = await donationsCollection
        .aggregate([
          { $unwind: "$donors" },
          {
            $group: {
              _id: null,
              totalAmount: { $sum: { $toDouble: "$donors.amount" } },
            },
          },
        ])
        .toArray();
      const donations =
        donationCount.length > 0 ? donationCount[0].totalAmount : 0;
    ...
    app.get("/donations-by-category", verifyToken, verifyAdmin,
      async (req, res) => {
        try {
          const donationsSummary = await donationsCollection
            .aggregate([
              { $unwind: "$donors" },
              {
                $group: {
    ...
    app.patch("/users/make-admin/:id", verifyToken, verifyAdmin, async (req, res) => {
        ...
        try {
          ...
          res
            .status(200)
            .send({ message: "User promoted to admin successfully." });
        } catch (error) {
    ...
    app.patch("/pets/status/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount === 0) {
          return res
            .status(404)
            .send({ error: "Pet not found or no changes made" });
        }

        res
          .status(200)
          .send({ message: "Pet status updated successfully", result });
      } catch (error) {
        // console.error('Error updating pet status:', error);
        res
          .status(500)
          .send({
            error: "Failed to update pet status. Please try again later.",
          });
      }
    });
    ...
    app.put("/users/:id", async (req, res) => {
      ...
      if (!email) {
        return res
          .status(400)
          .send({ message: "Email is required for update!" });
      }
  ...
  try {
    ...
    app.get("/pets", async (req, res) => {
      ...
      try {
        ...
        console.error(err);
        res.status(500).send("An error occurred while fetching pets.");
      }
    });
    ...
    app.put("/my-pets/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ error: "Failed to update pet. Please try again later." });
      }
    });
    ...
    app.post("/request-adoption", async (req, res) => {
      ...
      const { petId, adopterName, adopterMail, adopterPhone, adopterAddress } =
        adoptionData;
      if (
        !petId ||
        !adopterName ||
      ...
      try {
        ...
        if (existingRequest) {
          ...
          return res
            .status(400)
            .send({ message: "You have already requested to adopt this pet." });
        }
        ...
        res
          .status(201)
          .send({ message: "Adoption request succussfull", result });
      } catch (error) {
        // console.error('Error processing adoption request:', error);
        res
          .status(500)
          .send({ message: "Failed to process adoption request.", error });
      }
    });
    ...
    app.patch("/adopted/status/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount === 0) {
          return res
            .status(404)
            .send({ message: "Adoption request not fount!" });
        }
        if (status === "Accepted") {
    ...
    app.get(
      "/adoptRequests/byOwnerMail/:posterEmail",
      verifyToken,
      async (req, res) => {
        ...
        try {
          ...
          if (postedPets.length === 0) {
            return res
              .status(404)
              .send({ message: "No pets found posted by this user." });
          }

          const petIds = postedPets.map((pet) => pet._id.toString());
          // console.log('petIds: ', petIds);

          const requests = await adoptsCollection
          ...
          if (requests.length === 0) {
            return res
              ...
              .send({
                message:
                  "No adoption requests found for pets posted by this user.",
              });
          }
          ...
          res
            .status(500)
            .send({ message: "Failed to fetch adoption requests.", error });
        }
      }
    );
      ...
      try {
        ...
        if (requests.length === 0) {
          return res
            ...
            .send({
              message:
                "No adoption requests found for this pet by the provided email.",
            });
        }
    ...
    app.patch("/cencell/status/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to update request status.", error });
      }
    });
    ...
    app.delete(
      ...
      async (req, res) => {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ error: "Invalid donation campaign ID" });
        }
        try {
      ...
      async (req, res) => {
        ...
        try {
          ...
          if (result.modifiedCount === 0) {
            return res
              .status(404)
              .send({ error: "Campaign not found or no changes made" });
          }
          res.status(200).send(result);
        } catch (error) {
    ...
    app.patch(
      "/admin/donation-campaigns/:id/pause",
      verifyToken,
      verifyAdmin,
      async (req, res) => {
        const id = req.params.id;
        ...
        try {
          ...
          res
            .status(500)
            .send({ message: "Failed to update campaign status", error });
        }
      }
    );
    ...
    app.post("/donation-campaigns", async (req, res) => {
      ...
      if (!campaignData)
        return res.status(400).send({ message: "Campaign data not recieved" });
      try {
    ...
    app.put("/donation-campaigns/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ error: "Failed to update camp. Please try again later." });
      }
    });
    ...
    app.get("/my-donation-campaigns/:email", verifyToken, async (req, res) => {
      ...
      if (!query) {
        return res
          .status(404)
          .send({ message: "You don't added any campaigns, Please add one" });
      }
      try {
    ...
    app.post("/update-donation", async (req, res) => {
      ...
      try {
        ...
        const updateDonate = await donationsCollection.updateOne(campFilter, {
        ...
        if (updateDonate.modifiedCount === 0) {
          return res
            .status(500)
            .send({ message: "Failed to update donation!" });
        }
        ...
        res
          .status(500)
          .send({ message: "An error occurred during donation update." });
      }
    });
    ...
    app.post("/donors/refund", verifyToken, async (req, res) => {
      ...
      if (!id || !email) {
        return res
          .status(400)
          .send({ message: "CAmpId and email are required!" });
      }
      const query = { _id: new ObjectId(id), "donors.email": email };
      try {
        // retrieve specific donor's amount
        const camp = await donationsCollection.findOne(query, {
          projection: { "donors.$": 1 },
        });
